...
export default class QuoteDataTable extends Component {
  ...
  deleteQuote = async id => {
	...
	if(confirmDelete)
	{
		const res = await Axios.delete( `http://localhost:7070/admin/quote/${id}` );
	...
	return (
		...
		<tbody>
			...
			<th scope="coloumn">{item.id}</th>
				...
				<td>{item.serviceRequired.map( (service, index) =>
				{ return(
					<div key={index}>
						<p>{service}</p>
					</div>
				...
				<td>
					<div style={{width:"110px"}}>
						...
						{' '}
						<Button color="danger" onClick={() => this.deleteQuote(item.id)}>Delete</Button>
					</div>
				</td>
			</tr>
			); } ) }
